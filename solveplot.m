...
[t,y] = ode45('batchSimpleF1',tspan,y0,[],params);
...
%% Plot Derivatives
yp = yp(51:end,:); %first fifty or so elements are messed up by initial guess
tp = tp(51:end,:);
yp1 = yp(:,1);
yp2 = yp(:,2);
yp3 = yp(:,3);
yp4 = yp(:,4);
eqt = ones(length(tp),1)*tp(eq_cut);

figure(2)
plot(tp,yp1,'b',tp,yp2,'r',tp,yp3,'k',tp,yp4,'m')
tt = ...
 sprintf('Derivatives');
title(tt)
xlabel('t')
